...
def admin():
    ...
    for o in orders:
        ...
        btns = ""
        if o['status'] == 'new':
            btns = '<button onclick="cancelOrder(' + str(oid2) + ')" style="background:#ef4444;color:#fff;border:none;padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer;font-size:12px">Отменить</button>'
        elif o['status'] in ('printed','scan_ready'):
            btns = '<button onclick="reprintOrder(' + str(oid2) + ')" style="background:#1a3a5c;color:#fff;border:1px solid rgba(255,255,255,0.2);padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer;font-size:12px">Перепечатать</button>'
        fname = o['file_name'] or '-'
        ptype = o['print_type']
        pay = o['payment'] or '-'
        cat = o['created_at']
        rows += f"<tr><td>#{oid2}</td><td>{cat}</td><td>{fname}</td><td>{ptype}</td><td>{o['pages']} x {o['copies']}</td><td><b>{o['total']} с</b></td><td>{pay}</td><td style='color:{sc}'>{st}</td><td>{btns}</td></tr>"
...
async function reprintOrder(id){
  if(!confirm("Перепечатать заказ?")) return;
