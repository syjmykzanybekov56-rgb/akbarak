...
def admin():
    ...
    for o in orders:
        ...
        st = {'new':'Очередь','printed':'Готово','scan_ready':'Скан готов','no_file':'Без файла'}.get(o['status'], o['status'])
        rows += f"<tr><td>#{o['id']}</td><td>{o['created_at']}</td><td>{o['file_name'] or '-'}</td><td>{o['print_type']}</td><td>{o['pages']} x {o['copies']}</td><td><b>{o['total']} с</b></td><td>{o['payment'] or '-'}</td><td style='color:{sc}'>{st}</td></tr>"
...
<div class="tw"><table>
<thead><tr><th>#</th><th>Время</th><th>Файл</th><th>Тип</th><th>Объём</th><th>Сумма</th><th>Оплата</th><th>Статус</th></tr></thead>
<tbody>{rows}</tbody>
</table></div></body></html>"""
    return html
