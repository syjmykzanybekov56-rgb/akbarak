...
def admin():
    ...
    for o in orders:
        ...
        oid2 = o["id"]
        fname = str(o["file_name"] or "-")
        ptype = str(o["print_type"] or "-")
        pay = str(o["payment"] or "-")
        cat = str(o["created_at"] or "-")
        tot = str(o["total"])
        pgs = str(o["pages"])
        cps = str(o["copies"])

        if o["status"] == "new":
            btn = '<button onclick="cancelOrder(' + str(oid2) + ')" style="background:#ef4444;color:#fff;border:none;padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer">Отменить</button>'
        elif o["status"] in ("printed","scan_ready"):
            btn = '<button onclick="reprintOrder(' + str(oid2) + ')" style="background:#2a4f7c;color:#fff;border:none;padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer">Перепечатать</button>'
        else:
            btn = "-"

        rows += "<tr><td>#" + str(oid2) + "</td><td>" + cat + "</td><td>" + fname + "</td><td>" + ptype + "</td><td>" + pgs + "x" + cps + "</td><td><b>" + tot + " с</b></td><td>" + pay + "</td><td style='color:" + sc + "'>" + st + "</td><td>" + btn + "</td></tr>"
...
<div class="stats">
<div class="sc"><div class="sn">""" + str(len(orders)) + """</div><div class="sl">Всего заказов</div></div>
<div class="sc"><div class="sn">""" + str(len(today_orders)) + """</div><div class="sl">Сегодня</div></div>
<div class="sc"><div class="sn" style="color:#ef4444">""" + str(new_count) + """</div><div class="sl">В очереди</div></div>
<div class="sc"><div class="sn">""" + str(today_rev) + """ с</div><div class="sl">Выручка сегодня</div></div>
<div class="sc"><div class="sn">""" + str(total_rev) + """ с</div><div class="sl">Всего выручка</div></div>
</div>
<div class="tw"><table>
...
async function reprintOrder(id){
  if(!window.confirm("Перепечатать?")) return;
