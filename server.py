...
def upload():
    try:
        ...
        file_path = os.path.join(FILES_DIR, f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}")
        file.save(file_path)

        conn = sqlite3.connect(DB_PATH)
        cursor = conn.execute(
            "INSERT INTO orders (file_name, file_path, print_type, pages, copies, total, payment, status, created_at) VALUES (?,?,?,?,?,?,?,'new',?)",
            (file.filename, file_path, print_type, pages, copies, total, payment, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        )
